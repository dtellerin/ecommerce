...
const routerCalculoRandom = Router()
    ...
    process.on('message', (resultado, uuidv1) => {
    ...
    routerCalculoRandom.get('/', (req, res) => {
        ...
        if (req.query.cant == undefined || req.query.cant == '') {
            logger.info('[', uuidv1, ']: App Calculo sin cantidad definida, se toma por default 1e6 ') 
            rands = 100000000
        } else {
            logger.info('[', uuidv1, ']: App Calculo se hace el calculo para: ' + req.query.cant)
            rands = req.query.cant
        }

        const computo = fork(__dirname + '/src/router/calculo.js')
        computo.on('message', (msg, uuidv1) => {
            if (msg == 'listo') {
                computo.send(rands, uuidv1)
            } else {
                logger.info('[', uuidv1, ']: Mensaje del hijo: OK')
                res.json({ msg })
            }
        })
